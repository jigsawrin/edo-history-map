...
reference asset台帳は2件で、和田倉御門1件がpublished、馬場先御門1件がtechnical approved / publication shortlistedである。和田倉は公式JPEGを監査し、撮影補助物だけを除く保守的cropのPNGを公開している。raw / derived画像はGit管理外、publishedのpublic PNGだけを追跡する。candidateの`intendedUses=["reference-panel"]`、測地同期・tile・control pointに使わない制約は維持し、candidate自体のtechnical/publication状態をasset/displayと同一視しない。公開時は1717年と江戸後期表示（1849–1862）の年代差、資料名、東京都立中央図書館所蔵、部分・加工、パブリックドメイン、公式資料・利用案内URLをpanelと出典dialogに示す。
...
馬場先御門の現代位置調査では、[千代田区文化財サイト](https://www.edo-chiyoda.jp/knainobunkazai/bunkazaisign_hyochu_setsumeiban/1/5/53.html)が「皇居外苑1先、馬場先門交差点から皇居外苑方面の歩道植栽内」を案内し、[千代田区観光協会](https://visit-chiyoda.tokyo/app/spot/detail/237)が馬場先門橋を皇居外苑1に置き、石垣の一部が残ると説明している。公式Google Mapリンクの案内点は`[139.760527, 35.678426]`である。2026-07-27に公式ビューアの第2図「馬場先御門」を全画面表示し、標準ダウンロード画像`6194_02_(02).jpg`を取得した。高精細画像申込は使用していない。canonical originalはJPEG 3514×2500、211124 bytes、SHA-256 `5235617d8deb9a77cef9326a85f4e9b54bc7be9211d4179d4edf7d1ffbf12195`。原本外の撮影背景、資料番号札、カラーチャート、グレースケール、定規だけを`x=500, y=270, width=2450, height=1800`、回転0度で除去し、PNG 2450×1800、861237 bytes、SHA-256 `5b2f4e6fa4c33022aa0ba3265b821e43226b1804d0456f12f382ed2d5d6fd36c`をGit管理外へ作成した。独立再確認で馬場先御門表記、資料札、crop前後、図面本体・注記・方角・細部図・歴史的余白・採寸線・印記号・台紙の歴史的部分を再確認し、歴史情報の欠落がないためassetをrights approved / technical approved / publication shortlistedとした。private display catalogにはUI案内用trigger envelopeをtechnical in-review / publication shortlistedで登録するが、これは史跡境界、1717年図面coverage、georeferencing、control pointではない。runtime registry、public画像、公開UI、測地同期、tile、control pointは未登録である。
