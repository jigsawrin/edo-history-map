...
runtimeはprivate display catalogを配信・読込みせず、`src/historical-reference-panel-registry.json`の縮約entryを専用監査でcatalogへ照合する。現在はdisplay 2、rights approved 2、technical approved 1、technical in-review 1、published 1、shortlisted 1、runtime eligible 1、runtime registry 1である。
...
## 馬場先御門reference display候補

`tokyo-archive-4300033114-babasaki-gate-reference-display`は、technical approved /
publication shortlistedのreference assetを参照する、非公開の`reference-only` /
`reference-panel`候補である。`parentMapId`を持たず、display自体はtechnical in-review /
publication shortlistedに留める。後続PRでdisplayのtechnical approvalと公開可否を別途審査する。
...
公開UI、marker、overlay、tileへは接続しない。
