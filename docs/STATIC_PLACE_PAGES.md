# 静的地点一覧

## 目的と公開URL

静的地点一覧は、JavaScriptやCanvas地図を利用しない場合にも、承認済み地点の名称、
分類、補助情報、出典へ到達するためのHTML導線である。

- `/places/`: 地域選択、出典、プライバシー説明
- `/places/edo/`: 東京・江戸の1ページ目
- `/places/edo/page-2.html` ～ `page-88.html`: 東京・江戸の続き
- `/places/kyoto/`: 京都・幕末36件
- `/places/shiga/`: 滋賀・戦国36件
- `/themes/`: JavaScript不要の歴史テーマ索引

地図本体のツールバーと`noscript`から`/places/`へリンクする。一覧から地図へ戻る
リンクも各ページに置く。

## ページ分割

東京・江戸8,788件は1ページ100件、全88ページとし、最終ページは88件である。
JavaScriptなしでもHTMLの転送量とナビゲーション量を抑えつつ、一覧性を保つための
固定値である。京都・幕末36件は1ページに収める。ページ順と地点順は既存検索の
空検索順を正規化して決定し、名称の読みを推測しない。

## 生成と再現性

```bash
npm run build:static-places
npm run build:static-themes
npm run audit:static-links
```

`scripts/build-static-place-pages.mjs`は`public/data/`の4 GeoJSONと、京都の表示用語を
定義する`src/kyoto-place-presentation.json`だけを読み、`dist/places/`を生成する。
生成前に4データのSHA-256、Feature数、必須属性、座標範囲、出典IDを検証する。
入力、並び順、テンプレートが同じなら出力も同じで、時刻を埋め込まない。

`dist/places/manifest.json` schema 2には入力SHA、件数、ページ数、各HTML/CSSのSHA-256に加え、
テーマ21件、関係87件、地域横断5件、テーマHTML/CSS、地点逆リンク追加後のSHA-256を記録する。
生成物はビルド成果物でありGit管理しない。`npm run build`はViteビルド後に静的一覧を
生成し、リンク監査まで実行する。

## データと表示用語の整合

- 東京・江戸は`public/data/edo-places.geojson`の既存属性だけを表示する。
- 京都・幕末は`public/data/kyoto-bakumatsu-places.geojson`と既存の出典レジストリを使う。
- 京都の分類、位置根拠、現地状態、位置精度の表示名は、地図カード、検索結果、静的一覧で
  同じJSON定義を共有する。
- 新しい史実、読み、座標、要約、分類は生成処理で推測・追加しない。

## EDO Derived Static pilot

EDO地点はsource GeoJSONをparseした後、source由来legacy key・anchor・名称sort・page番号・page slotを先に確定し、その後で`scripts/edo-static-place-projection.json`の表示差分だけを適用する。projectionはbuild専用であり`public/`や`dist/`へ配信しない。現在は8,788件がstatic-page applicable、overridesは0件である。

approved renameは記事見出しの表示名だけを変更し、legacy key、anchor、sort、page、slot、CODH URL、page範囲表示、canonical URLを変更しない。approved hideはrecordを配列から除外せず、同じanchor/page/slotへ「この地点は現在、表示対象外です。」というgeneric tombstoneを残す。tombstoneには元名称、分類、切絵図、個別CODH URL、hide理由、private curation情報を出力しない。ページ全体のCODH attributionとlicenseは維持する。

`legacyLayoutSha256`は全EDO地点のsourceIndex、legacy key、anchor、page番号、page slotを固定する。source SHAまたはlayout SHAが変化した場合、projectionを自動更新せずbuild/auditを失敗させる。現baselineではprojection適用前後のStatic HTML、URL、anchor、manifestをbyte-for-byte同一に保つ。

## HTML・リンクの安全性

データ由来の文字列は制御文字を除去し、`&`、`<`、`>`、`"`、`'`をHTMLエスケープする。
外部URLはHTTPS、許可済みホストとパス、認証情報なしを検証する。外部リンクは
`target="_blank" rel="noopener noreferrer"`と「外部サイト」の可視文言を持つ。

静的ページのCSPはスクリプト、画像、フォント、外部接続を禁止する。`script`、フォーム、
画像、iframe、インラインイベント属性を生成しない。テーマ索引と歴史年表も同じ制約を持つ。監査はさらに、内部リンクとアンカー、
重複ID、空リンク、見出し順、現在ページ、前後ページ、件数、CSS、manifestの全ハッシュを
検査する。地点91ページ、テーマ26ページ、年表3ページの計120 HTMLを一括監査する。

年表生成は京都・滋賀地点ページだけへ「関連する歴史年表」を最大5件追加する。名称一致ではなく
年表の`relatedPlaces`を逆引きし、日付、項目名、関係説明、時代別年表アンカーを表示する。
EDO地点には追加しない。manifest schema 3は年表件数、track・type・precision・calendarBasis内訳、
地点・テーマ関係、3ページのHTML SHA、キュレーションSHA、逆リンク数、CSS SHAを既存情報に加える。

## アクセシビリティとプライバシー

各ページは日本語`lang`、固有title・description、スキップリンク、`main`、階層的見出し、
通常リンク、現在ページのテキストを持つ。CSSはキーボードフォーカス、200％拡大、
狭い画面、強制カラー、印刷を考慮する。

一覧はJavaScript、位置情報、フォーム、Cookie、ブラウザストレージ、外部画像・フォント、
アクセス解析を使わない。GitHub Pagesによる配信と、利用者が明示的に選んだ出典サイトへの
移動は通信を伴う。

## 制約

静的一覧は地点の文字情報と出典への導線であり、地図上の距離、方向、重なり、周辺環境、
Canvasレイヤーを再現しない。検索フォームも提供しない。すべてのスクリーンリーダーや
物理モバイル実機での完全な動作を保証するものではない。
