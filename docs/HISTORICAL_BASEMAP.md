...
`EraDefinition`は年代ID・表示名・年範囲だけを管理する。表示レイヤー、データセット、出典、推定表示の注意文、基図モードは地域パックの`RegionEraDefinition`が管理する。UIへ表示する年代は、選択地域の`enabledEraIds`と有効なバインディングの両方を満たすものだけである。現在の本番用地域パックは東京・江戸のみで、`edo-early`は無効のまま維持する。
