...
`EraDefinition`は年代ID・表示名・年範囲だけを管理する。表示レイヤー、データセット、出典、推定表示の注意文、基図モードは地域パックの`RegionEraDefinition`が管理する。UIへ表示する年代は、選択地域の`enabledEraIds`と有効なバインディングの両方を満たすものだけである。現在の本番用地域パックは東京・江戸と京都の2件で、`edo-early`は無効のまま維持する。
...
京都・幕末は、現代の地理院タイルとプロジェクト独自編集の史跡36地点だけを表示する。
古地図背景、推定ポリゴン・ライン、水域、街道、藩邸敷地、鳥羽・伏見の戦闘範囲は
導入していない。東京・江戸のデータや装飾も再利用しない。カテゴリは色だけでなく
円の破線パターンと文字付き凡例を併用し、すべてLeaflet Canvasへ描画する。

地点座標は公的資料の世界測地系座標を小数6桁へ変換したもので、現在の碑・建物・
記念地と幕末当時の現場が一致しない場合を `LocationBasis` と
`HistoricalSiteStatus` で区別する。現代・幕末の切替は同じ `L.Map` を再利用し、
中心・ズーム・現在地を維持する。
...
5. `public/data/historical-rasters/` 配下の全タイルとSHA-256・bytes・寸法をJSON manifestへ
   記録し、台帳の `tile_manifest_sha256` と `original_sha256` でmanifestと原本を固定する。
...
`HistoricalRasterDefinition`はregion/era/source/attribution、固定ローカルタイルとmanifest、
PNGまたはlossless WebP、256px、zoom、bounds、不透明度、変換方式、基準点数、推定・最大誤差、
原本年代、対象範囲、注意文、seam policy、priority、review statusを持つ。sourceと定義の
両方がapprovedの場合だけ地域パックから参照できる。

画像は1シート1パックとし、`single-sheet`、`manual-selection`、`fixed-priority`以外の
境界処理を行わない。同じLeaflet mapの中心・zoomを維持して切り替え、無理なシームレス結合、
透明合成による境界隠し、自動fitBounds、欠損生成はしない。現在のレジストリは空なので、
江戸後期は`baseMode: reconstructed`を維持し、CSPも変更していない。
