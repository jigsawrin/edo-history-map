...
`fact-verification`は事実確認、`text-reuse`は文章の再利用権限であり、相互に代用しない。fact-verification sourceの文章表現を公開本文へ転載・翻案してはならない。`editorial-summary`はtext reuse sourceのreproduction、modification、summarizationがすべて`allowed`である必要がある。
